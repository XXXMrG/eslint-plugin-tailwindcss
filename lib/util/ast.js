...
const splitWithIndex = require('./splitWithIndex');
// context.parserPath
...
function extractClassnamesFromValue(classStr) {
  ...
  let parts = splitWithIndex(classStr, separatorRegEx);
  const headString = parts[0] && parts[0].value;
  const tailString = parts[parts.length - 1] && parts[parts.length - 1].value;
  ...
  let headSpace = separatorRegEx.test(headString);
  let tailSpace = separatorRegEx.test(tailString);
  const isClass = (_, i) => (headSpace ? i % 2 !== 0 : i % 2 === 0);
  const isNotClass = (_, i) => (headSpace ? i % 2 === 0 : i % 2 !== 0);
  const classNameMap = parts.filter(isClass);
  let classNames = classNameMap.map((part) => part.value);
  let classNamesIndex = classNameMap.map(part => part.index);
  let whitespaces = parts.filter(isNotClass).map((part) => part.value);
  return { classNames: classNames, whitespaces: whitespaces, headSpace: headSpace, tailSpace: tailSpace, classNamesIndex };
}
