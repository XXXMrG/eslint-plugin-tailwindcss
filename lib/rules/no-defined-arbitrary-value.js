  ...
  create: function (context) {
    ...
    const parseForArbitraryValues = (node, arg = null) => {
      ...
      classNames.forEach((cls, idx) => {
        ...
        if (arbitraryValue) {
            const key = arbitraryValue[1];
            if (map.has(key)) {
                forbidden.push({
                    arbitraryValue: key,
                    definedToken: map.get(key),
                    fixedClass: originalClassNamesValue.replaceAll(`[${key}]`, map.get(key)[0])
                });
            }
        }
      });
      ...
      forbidden.forEach((data) => {
        context.report({
          ...
          data,
          fix: function(fixer) {
            return fixer.replaceTextRange([start, end], data.fixedClass)
          }
        });
      });
    };
